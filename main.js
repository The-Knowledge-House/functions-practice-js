...
function classMate(array) {
  for (let i = 0; i < array.length; i++) {
    console.log(array[i] + " is in the Web Development class");
  }
}
classMate(students);
...
function calculateDogAge(pupAge, humanAge) {
  ...
  console.log(
    "Your dog is " + Math.floor(humanAge) + " years old in human years!"
  );
}

calculateDogAge(3, 60);
...
function calCircumferenceArea(radius) {
...
function celsiusToFahrenheit(celsius) {
  let celsiusToFah = celsius * (9 / 5) + 32;
  console.log(celsius + " in celsius is " + celsiusToFah + " in Farenheit");
}

celsiusToFahrenheit(34);

function fahrenheitToCelsius(farenheit) {
  let fahToCel = (farenheit - 32) * (5 / 9);
  console.log(farenheit + " in farenheit is " + fahToCel + " in Celsius");
}
